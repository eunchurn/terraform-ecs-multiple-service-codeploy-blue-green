...
resource "aws_ssm_parameter" "POSTGRESQL_CONNECTION_URI" {
  name        = "/${var.application_name}/${terraform.workspace}/POSTGRESQL_CONNECTION_URI"
  description = "POSTGRESQL_CONNECTION_URI"
  type        = "SecureString"
  value       = "postgresql://${jsondecode(data.aws_secretsmanager_secret_version.db_secret.secret_string)["username"]}:${jsondecode(data.aws_secretsmanager_secret_version.db_secret.secret_string)["password"]}@${jsondecode(data.aws_secretsmanager_secret_version.db_secret.secret_string)["host"]}:${jsondecode(data.aws_secretsmanager_secret_version.db_secret.secret_string)["port"]}/authdb"
  overwrite   = true
}
...
resource "aws_ssm_parameter" "KAKAO_KEY" {
  name        = "/${var.application_name}/${terraform.workspace}/KAKAO_KEY"
